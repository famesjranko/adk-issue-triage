# ADK notes from building this, not from reading a doc
...
fires as a `DeprecationWarning` on import. `adk.dev` still documents only the
old API. The class ships in the package before the docs describe it.

So the "SequentialAgent / ParallelAgent / LoopAgent" triad that every ADK
tutorial teaches is the previous generation. I implemented both here:
...
I found one constraint the hard way. A `Workflow` cannot currently be a sub-agent
...
the agent code makes it visible. That is the entire reason `CostMeterPlugin`
...
`JoinNode` is the AND-join, with `_requires_all_predecessors = True`. I fixed it by
routing all three branches into one `JoinNode` and edging that to `priority`.

**The failure was loud.** A missing state key raised
...
The control flow is fetch, then three independent analyses, then prioritise, then
judge readiness. That order is fixed and known before any request arrives, and it
is encoded as graph edges. Nothing asks the model to rediscover it on every run.
...
**v1.** A `before_tool_callback` refused `apply_labels` unless session state
...
the substance. The run never stopped. The callback returned an error string,
and the model was free to narrate around it.

**v2.** ADK has the real gate built in, and I only found it by reading Google's
...
arguments, so the policy is a function of what is being written:
...
- Authorisation (`require_confirmation`) suspends the run. It is a runtime
  feature, not a callback. Duplicating it in the callback would only add a second,
  weaker copy.

**Prompting the model into misbehaving proves nothing about a guard.** In one
attempt the model called `list_labels` first, noticed `area/frontend` did not exist, and declined on its own. Good behaviour,
...
select which. Both default to flash-lite today. The split exists so the
...
ceiling or let the slow one blow its quota. The fan-out issues three calls at
once, which is the burst that trips the limit. So the limiter belongs on the
Runner, where it sees the whole tree, not on any one agent.

`gemini-3.8-flash` also returned 503 UNAVAILABLE roughly 1 run in 3 under free
tier. Model choice is an availability decision as well as a quality one.
...
window, but it does not fix this. One process at a time fixes it, or a limiter that
lives outside the process. Worth knowing before anyone runs an eval suite
...
label rather than counting them as misses. Of 39 issues, 20 carry no readiness
...
and a single `response_match_score` would have hidden it.
...
wrote it down as a result. That is the mistake worth not repeating. The control channel
...
every grounded repeat the same one, which is the shape a deterministic
...
cadence, scaling profile, or trust boundary**. None of those is true here.
...
which meant the deployed agent could not fetch an issue. I rewrote it against the
...
first two images. I deleted both images and their source zips and treated the
key as burned; the folder now carries a `.gcloudignore`. Worth checking on any ADK
...
and died in the second. Every retry after that point fails, so I stopped the
run rather than let it score exhausted calls as misses.

The per-minute retry did its job before that. `RetryConfig(max_attempts=3)` on
the node absorbed a real 429 from a concurrent request, and the case completed. The daily cap is not retryable, and no client-side limiter can see
...
is what a newsroom tool would call, and it is where the approval gate
...
The pricing page says Google uses free-tier prompts to improve its products.
`ALLOWED_REPOS` in `tools/github.py` therefore hard-limits the
